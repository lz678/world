...
const actions = {
    ...
    return Api.getUserInfo()
      .then(data => {
        ...
        if (data.code === 1) {
          ...
          const obj = Object.assign(userInfo, data.data)
  ...
  // 上传文件
  uploadFileone ({ commit, state }, fileList) {
    Toast.loading({
      mask: true,
      duration: 0,
      message: '上传中...'
    ...
    return Promise.all(arr)
      .then(data => {
        ...
        resulte.photo1=resulte.data.url
        if (resulte.code === 1) {
  ...
  uploadFiletwo ({ commit, state }, fileList) {
    Toast.loading({
      mask: true,
      duration: 0,
      message: '上传中...'
    ...
    return Promise.all(arr)
      .then(data => {
        ...
        const resulte = data[0]
        console.log(resulte);
        resulte.photo2=resulte.data.url
        // resulte.can=resulte.data.url
        if (resulte.code === 1) {
          const res = data.map(it => it.data.url)
  ...
  uploadFilethree ({ commit, state }, fileList) {
    Toast.loading({
      mask: true,
      duration: 0,
      message: '上传中...'
    ...
    return Promise.all(arr)
      .then(data => {
        ...
        const resulte = data[0]
        console.log(resulte);
        resulte.photo3=resulte.data.url
        // resulte.can=resulte.data.url
        if (resulte.code === 1) {
          const res = data.map(it => it.data.url)
